...
class TestAbstractEntry:
    ...
    def test_get_next_or_previous_without_default_ordering(self):
        ...
        assert (
            entry1.get_next_or_previous(is_next=True, ordering=["-created"]) == entry3
        )

        assert (
            entry2.get_next_or_previous(is_next=False, ordering=["-created"]) == entry3
        )
        assert entry2.get_next_or_previous(is_next=True, ordering=["-created"]) is None

        assert (
            entry3.get_next_or_previous(is_next=False, ordering=["-created"]) == entry1
        )
        assert (
            entry3.get_next_or_previous(is_next=True, ordering=["-created"]) == entry2
        )
    ...
    def test_get_next_or_previous_with_default_ordering(self):
        ...
        # 传入额外参数
        assert (
            entry2.get_next_or_previous(
                is_next=False, value_fields=["rank"], rank__gt=2
            )
            is None
        )
