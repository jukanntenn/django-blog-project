...
class CommentViewSet(mixins.ListModelMixin,
    ...
    def _check_params(self):
        ...
        if ctype is None or object_pk is None:
            raise ValidationError({'detail': "Missing content_type or object_pk field."})
        try:
            ...
            raise ValidationError({'detail': "Invalid content_type value: %r" % escape(ctype)})
        except AttributeError:
            raise ValidationError({'detail': 'The given content-type %r does not resolve to a valid model.'},
                                  )
        except ObjectDoesNotExist:
            raise ValidationError({'detail': "No object matching content-type %r and object PK %r exists." % (
                escape(ctype), escape(object_pk))})
        except (ValueError, serializers.ValidationError) as e:
            raise ValidationError({'detail': "Attempting go get content-type %r and object PK %r exists raised %s" % (
    ...
    def _check_form(self):
        ...
        if self.form.security_errors():
            raise ValidationError({'detail': "The comment form failed security verification: %s" %
    ...
    def get_queryset(self):
        ...
        root_comments = BlogComment.objects.root_nodes().filter(content_type=target_ct, object_pk=target.pk)
        qs = root_comments.get_descendants(include_self=True).select_related('user', 'parent__user')
        # Todo: 使用子查询优化头像的获取
        qs = qs.prefetch_related('user__socialaccount_set', 'parent__user__socialaccount_set')
    ...
    def perform_create(self, serializer):
        ...
        for (receiver, response) in responses:
            if response is False:
                return Response(
                    data={'detail': "comment_will_be_posted receiver %r killed the comment" % receiver.__name__},
