...
class CourseDetailView(SetHeadlineMixin, generic.DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if first_material:
            context['next'] = {'title': first_material.title, 'url': first_material.get_absolute_url()}
        return context
    ...
    def get_context_data(self, **kwargs):
        ...
        if material_prev:
            context['prev'] = {
                ...
                'url': reverse('courses:material_detail',
                               kwargs={'pk': material_prev['id'], 'slug': material_prev['course__slug']})
            }
        if material_next:
            context['next'] = {
                'title': material_next['title'],
                'url': reverse('courses:material_detail',
                               kwargs={'pk': material_next['id'], 'slug': material_next['course__slug']})
            }
        return context
